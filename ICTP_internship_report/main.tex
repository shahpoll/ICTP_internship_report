...
\label{sec:qe-basics}
With the toolchain validated, we refreshed the ``stretching'' exercises using the updated Mac workflows hosted in the QE\_stretching\_macm4 repository\footnote{\url{https://github.com/shahpoll/QE_stretching_macm4}} (\path{/Users/pollob/qe_basics}). Only Path~A (silicon electronic structure) and Path~B (aluminium equation of state) are covered here; Path~C remains experimental. Both paths run under the same \textsc{pw.x} build described earlier and retain provenance captures (environment, pseudopotential checksums, timing probes) under each path's \texttt{work/} directory.

\subsection{Path A — Silicon bands, DOS, and PDOS}
The refreshed Path~A workflow (\path{pathA\_si\_mac}) follows the standard QE sequence: SCF on an $8\times8\times8$ mesh, NSCF on a $12\times12\times12$ mesh, and band tracing along the FCC high-symmetry path, followed by \texttt{bands.x}, \texttt{dos.x}, and \texttt{projwfc.x}. Smearing is removed for SCF; tetrahedra are used for NSCF to sharpen the indirect gap. Full run scripts are provided in Appendix~\ref{app:repro}. Performance on eight ranks completes SCF in $2.4$~s, bands in $30.3$~s, NSCF in $8.2$~s, and DOS/PDOS in $\sim6.7$~s. The indirect gap from \texttt{si.bands.dat.gnu} is $0.56$~eV, consistent with PBE expectations and the benchmarking suite. Provenance (environment, pseudopotential checksums, timing probes) remains in \texttt{work/}.
...
the provided harvest/fit scripts (Appendix~\ref{app:repro}) rebuilds all tables and figures from the raw QE outputs in \texttt{work/}.
  ...
  The BM3 fit converges to $a_0 = 4.0373$~\AA, $B_0 = 76.20$~GPa, $B^\prime = 4.830$ with an RMS of $1.48$~meV/atom (values mirrored in \texttt{work/eos\_fit.json} and \texttt{latex/eos\_table.tex}). Figures~\ref{fig:pathB-eos} and \ref{fig:pathB-residuals} show the energy--volume curve and residuals; the residual energy distribution exhibits a structured oscillation within $\pm2$~meV/atom, negligible compared with the binding energy, confirming that a third-order Birch--Murnaghan form sufficiently captures the lattice anharmonicity near equilibrium.
...
The aluminium equation-of-state workflow resides in \path{/home/pollob/qe_basics/pathB\_eos\_al} on the server. It packages a resume-safe 15-point sweep (0.90--1.18~$a_0$) with a $24\times24\times24$ mesh, $40/320$~Ry cutoffs, and MV smearing $\sigma=0.02$~Ry, driven by the GPU-enabled QE~7.4.1 build. The NVIDIA HPC SDK/HPC-X environment is loaded inside \texttt{scripts/run\_eos.sh} before launching \texttt{mpirun -np 6} with \texttt{OMP\_NUM\_THREADS=1}; completed scales are skipped automatically.
...
The residual energy distribution exhibits a structured oscillation within $\pm2$~meV/atom, negligible relative to the binding energy; this supports the adequacy of the third-order Birch--Murnaghan description of lattice anharmonicity near equilibrium in the server run.

\section{Mac mini vs Server Performance Comparison}
\label{sec:comparison}
We reran the head-to-head measurements using the consolidated repository\footnote{\url{https://github.com/shahpoll/qe_benchmarking}} (\texttt{qe\_benchmarking}). Each run records the launcher (\texttt{command.log}), environment snapshot (\texttt{env.log}), hardware inventory (\texttt{hardware.log}), QE output (\texttt{pw.out}), and timing probe (\texttt{time.log}). Aggregated wall, CPU, and core-second totals live in \texttt{runs/local\_summary.tsv}, with slowdown ratios summarised in \texttt{runs/comparison\_summary.txt}. The larger MgO cell and multi-volume aluminium sweep reduce the risk of timings being dominated by MPI or CUDA start-up, addressing the concerns raised after the initial tutorial-sized benchmarks. The matrix of workloads is visualised in Figure~\ref{fig:benchmark-matrix}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{workloads_overview.png}
  \caption{Benchmark suite tracked in \texttt{qe\_benchmarking}: bulk silicon, bulk magnesium oxide, and a nine-point aluminium EOS sweep, each executed on the Mac (1- and 8-rank CPU) and the server (6-rank CPU and 4-rank GPU builds).}
  \label{fig:benchmark-matrix}
\end{figure}

\subsection{Physics sanity checks}
Before reading timings, we validated that the electronic-structure outputs remain physically sensible. The silicon bulk run yields a PBE gap of $0.560$~eV (Figure~\ref{fig:si-band-dos}), matching the $0.5597$~eV reported in \texttt{runs/si\_bulk\_bands/summary.txt}. Magnesium oxide returns a $4.74$~eV gap (Figure~\ref{fig:mgo-band-dos}), consistent with the usual PBE underestimate relative to experiment. The aluminium EOS fit (Figures~\ref{fig:al-eos-fit-curve} and \ref{fig:al-eos-per-volume}) gives $B_0 \approx 26$~GPa and $B_0' \approx 6.35$ per \texttt{runs/al\_eos\_fit/fit\_summary.txt}; the deliberately broad MV smearing ($0.02$~Ry) keeps the benchmark stable at the cost of a softer bulk modulus than literature values.
...
The Apple M4 port described in Section~\ref{sec:m4section} remains stable. The refreshed \texttt{qe\_benchmarking} runs converged cleanly: the silicon and magnesium oxide workflows reproduced the expected indirect gaps, and no manual intervention was needed during SCF, NSCF, bands, or DOS stages. Mac-side timings from \texttt{runs/local\_summary.tsv} show $44.9$~s ($19.4$~s) for the silicon bulk case on one (eight) ranks, $40.5$~s ($26.8$~s) for MgO, and $11$--$16$~s ($9$--$10$~s) per aluminium EOS point on one (eight) ranks, confirming sub-minute turnaround on commodity hardware.

\subsection{Stretching workflows on macOS and the server}
Replaying the Path~A and Path~B exercises locally and on the stretching server confirmed functional parity across platforms. The macOS runs produced high-quality bands, DOS, and PDOS plots (Figures~\ref{fig:pathA-bands}--\ref{fig:pathA-dos}), while the server runs captured the same artefacts with GPU-enabled executables (Figures~\ref{fig:server-pathA-bands}--\ref{fig:server-pathB-residuals}). Provenance bundles in each repository (\texttt{work/env.txt}, \texttt{work/runlog.txt}, \texttt{work/pp\_checksums.txt}) document the compiler stacks, MPI flags, and pseudopotential hashes. The tutorial-style Path~B aluminium EOS sweep converged at $a_0 = 4.0373$~\AA{} and $B_0 = 76.20$~GPa on both machines (Figures~\ref{fig:pathB-eos} and \ref{fig:server-pathB-residuals}), while the benchmarking sweep in Section~\ref{sec:comparison} used MV smearing to prioritise timing stability and therefore reports a softer $B_0 \approx 26$~GPa.

\subsection{Cross-platform performance comparison}
The comparison study in Section~\ref{sec:comparison} quantified the refreshed performance gap between the macOS workstation and the Ubuntu server. Slowdown factors derived from \texttt{runs/comparison\_summary.txt} and plotted in Figures~\ref{fig:timing-si}--\ref{fig:timing-al} show that the server CPU build is $15$--$17\times$ slower and the GPU build is $10$--$12\times$ slower than the Mac on matched inputs. The underlying wall times (silicon $19$~s vs $203$--$332$~s, MgO $27$~s vs $316$--$459$~s, aluminium EOS $6$--$10$~s vs $103$--$153$~s) confirm that MPI/GPU overheads dominate the server runs despite the presence of an RTX~4090.

\begin{table}[H]
  \centering
  \begin{tabularx}{0.9\linewidth}{@{}lXXX@{}}
    \toprule
    Parameter & Mac mini M4 & Ubuntu Server & Reference (Literature) \\
    \midrule
    Si indirect band gap (eV) & 0.56 & 0.582 & $\sim$1.1 (exp., PBE underestimates) \\
    Si Fermi level (eV) & 6.43 & 6.21 & --- \\
    Al lattice constant $a_0$ (\AA) & 4.0373 & 4.0373 & 4.0495 (300 K exp.) \\
    Al bulk modulus $B_0$ (GPa) & 76.20 & 76.02 & $\sim$76 (300 K exp.) \\
    \bottomrule
  \end{tabularx}
  \caption{Summary of calculated physical properties across macOS and Ubuntu runs; literature values shown for context.}
  \label{tab:phys-summary}
\end{table}
...
The comparative analysis highlights a distinct architectural trade-off. While GPU-accelerated servers are essential for large-scale research, our benchmarks demonstrate that the Mac mini M4 provides superior efficiency for the iterative, small-cell calculations used in training. The M4 ecosystem eliminated the significant MPI and CUDA startup latency observed on the server, resulting in a more responsive workflow for the studied test cases. Future work will focus on extending these pipelines to complex magnetic systems (Path C) and exploring potential Metal-backend acceleration for QE to bridge the gap in large-scale computation.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Provenance bundles}: environment snapshots, pseudopotential hashes, and command logs stored in each workflow's work/ directory (local and server).
  \item \textbf{Comparison data and figures}: logs, timing tables, and plots in qe\_benchmarking\\(runs/local\_summary.tsv, runs/comparison\_summary.txt, runs/plots/*.png).
...
\begin{itemize}
  \item \href{https://github.com/shahpoll/qe_macos15_build}{\texttt{qe\_macos15\_build}} — scripts and documentation for building QE~7.4.1 on macOS~15.
  ...
  \item \href{https://github.com/shahpoll/qe_benchmarking}{\texttt{qe\_benchmarking}} — Consolidated benchmarking suite (logs, timings, band/DOS checks, and plots) for the Mac-versus-server study.
\end{itemize}
